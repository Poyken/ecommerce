    ...
    try {
      ...
      if (admins.length > 0) {
        ...
        adminIds.forEach((adminId) => {
          this.notificationsGateway.sendNotificationToUser(
            adminId,
            adminNotification, // Client expects just the data, ID is generated by DB but for real-time we might miss ID if we don't query it back.
            // Actually broadcastToUserIds creates records.
            // To be precise we should emit what was created.
            // But broadcastToUserIds returns batch info.
            // For simplicity, we construct a "fake" notification object or just rely on the user refreshing,
            // BUT the gateway uses it to display the popup.
            // Ideally we should emit: { ...adminNotification, id: 'temp', createdAt: new Date() }
            {
              ...adminNotification,
              id: `temp-${Date.now()}`,
              createdAt: new Date(),

...
async function runTest() {
  ...
  try {
    ...
    const prodsRes = await axios.get(`${API_URL}/products`, { headers: HEADERS });
    const product = prodsRes.data.data?.[0];
    
    if (!product) {
      throw new Error('No products found on tenant!');
    }
    console.log(`${LOG_PREFIX}    ✅ Found product: ${product.name} (${product.minPrice} VND)`);
    ...
    console.log(`${LOG_PREFIX}    🎉 ORDER PLACED SUCCESSFULLY! ID: ${orderId}`);
    console.log(`${LOG_PREFIX}       Total: ${orderRes.data.data.totalAmount} VND`);
    ...
    const adminOrderRes = await axios.get(`${API_URL}/orders/${orderId}`, { headers: adminHeaders });
    ...
    console.error(`${LOG_PREFIX} ❌ TEST FAILED:`, error.response?.data || error.message);
    process.exit(1);
  }
}

...
export function HeaderBlock({ 
    ...
    // Normalize props
    const isTransparent = styles?.transparent !== undefined ? styles.transparent : transparent;
    ...
    const headerUtils = utils || [
        ...(showSearch ? [{ icon: "Search", label: "Search", href: "/search" }] : []),
        ...(showCart ? [{ icon: "ShoppingCart", label: "Cart", href: "/cart" }] : []),
        ...
        >
            <div 
                ...
                style={{ 
                    ...
                    backgroundColor: !isTransparent ? (bgColor || "rgba(255, 255, 255, 0.8)") : undefined,
                    color: textColor
                }}
            >
                ...
                <nav className={cn(
                    ...
                    {navLinks.map((link) => (
                        <Link
                            ...
                            className={cn(
                                "flex items-center gap-2 text-[11px] font-bold tracking-[0.2em] uppercase transition-colors relative after:absolute after:bottom-[-4px] after:left-0 after:h-[1px] after:bg-primary after:transition-all after:duration-300",
                                pathname === link.href 
                                    ? "text-primary after:w-full" 
                                    : "hover:text-primary after:w-0 hover:after:w-full"
                            )}
                            style={{ color: pathname !== link.href && textColor ? textColor : undefined }}
                        >
                ...
                <div className="flex items-center gap-2 md:gap-4 flex-none">
                    {headerUtils.map((util, idx) => (
                        ...
                        >
                            <FlexibleIcon source={util.icon} className={!!textColor ? "" : "text-foreground/70"} />
                        </Link>
                    ))}

...
export function NewsletterBlock({ title, description, styles }: NewsletterBlockProps) {
  return (
    <m.section
    ...
    >
        <div className="relative overflow-hidden bg-foreground/2 rounded-[3rem] p-12 md:p-24 text-center border border-foreground/5 shadow-2xl backdrop-blur-xl">
          ...
          {title && <h2 className="text-3xl md:text-4xl font-black tracking-tighter uppercase italic mb-4 relative z-10">{title}</h2>}
          {description && <p className="text-muted-foreground mb-8 relative z-10 max-w-2xl mx-auto">{description}</p>}
